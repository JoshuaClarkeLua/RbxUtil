...
--[=[	
	...
	Defers the given function to the next frame.
	Cleanup is removed when function finishes execution.

	```lua
	trove:Defer(function()
		print("Deferred to next frame")
	end)
	```
]=]
function Trove.Defer(self: TroveInternal, fn: (...any) -> (), ...: any): thread
	if self._cleaning then
		error("cannot call trove:Defer() while cleaning", 2)
	end
	local cleanupFn
	local thread = task.defer(function(...)
		fn(...)
		self:_findAndRemoveFromObjects(cleanupFn, false)
	end, ...)
	cleanupFn = self:Add(function()
...
--[=[
	...
	Delays the given function by the given amount of time.
	Cleanup is removed when function finishes execution.

	```lua
	trove:Delay(1, function()
		print("Delayed by 1 second")
	end)
	```
]=]
function Trove.Delay(self: TroveInternal, delay: number, fn: (...any) -> (), ...: any): thread
	if self._cleaning then
		error("cannot call trove:Delay() while cleaning", 2)
	end
	local cleanupFn
...
--[=[
	...
	Fires the function when the event is fired. Any event arguments are passed to the function.
	Cleanup is removed when function finishes execution.

	```lua
	trove:Event(workspace.ChildAdded, function(instance)
		print(instance.Name .. " added to workspace")
	end)
	```
]=]
function Trove.Event(self: TroveInternal, event: any & { Connect: (...any) -> ( any & { Disconnect: (...any) -> () }) }, fn: (...any) -> ()): thread
	if self._cleaning then
		error("cannot call trove:Delay() while cleaning", 2)
	end
	local cleanupFn
